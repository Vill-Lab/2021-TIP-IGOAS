...
class Engine(object):
    ...
    def _evaluate(self, epoch, dataset_name='', queryloader=None, galleryloader=None,
        ...
        qf, qf_weight, q_pids, q_camids = [], [], [], [] # query features, query person IDs and query camera IDs
        for batch_idx, data in enumerate(queryloader):
            imgs, pids, camids = self._parse_data_for_eval(data)
            if self.use_gpu:
                imgs = imgs.cuda()
            end = time.time()
            features, weight = self._extract_features(imgs)
            batch_time.update(time.time() - end)
            features = features.data.cpu()
            qf.append(features)
            qf_weight.append(weight)
            q_pids.extend(pids)
            q_camids.extend(camids)
        qf = torch.cat(qf, 0) #([3368, 2048])
        qf_weight = torch.cat(qf_weight, 0)  # ([3368, 6])
        q_pids = np.asarray(q_pids)
        q_camids = np.asarray(q_camids)
        print('Done, obtained {}-by-{} matrix'.format(qf.size(0), qf.size(1)))
        print('Done, obtained {}-by-{} weight matrix'.format(qf_weight.size(0), qf_weight.size(1)))

        print('Extracting features from gallery set ...')
        gf, gf_weight, g_pids, g_camids = [], [], [], [] # gallery features, gallery person IDs and gallery camera IDs
        for batch_idx, data in enumerate(galleryloader):
            ...
            features = features.data.cpu()
            gf.append(features)
            gf_weight.append(weight)
            g_pids.extend(pids)
            g_camids.extend(camids)
        gf = torch.cat(gf, 0)
        gf_weight = torch.cat(gf_weight, 0)
        g_pids = np.asarray(g_pids)
        g_camids = np.asarray(g_camids)
        print('Done, obtained {}-by-{} matrix'.format(gf.size(0), gf.size(1)))
        print('Done, obtained {}-by-{} weight matrix'.format(gf_weight.size(0), gf_weight.size(1)))
        ...
        if rerank:
            ...
            distmat_qq = metrics.compute_distance_matrix(qf, qf, qf_weight, qf_weight, dist_metric)
            distmat_gg = metrics.compute_distance_matrix(gf, gf, gf_weight, gf_weight, dist_metric)
