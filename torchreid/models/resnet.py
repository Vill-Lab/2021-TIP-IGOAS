...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.classifier_g = nn.Linear(512, num_classes)

        # part branch
        self.res_part2 = Bottleneck(2048, 512)
        self.part_maxpool = nn.AdaptiveMaxPool2d((1,1))
        self.batch_drop = BatchDrop(self.height_ratio, self.width_ratio)
        self.reduction_p = nn.Sequential(
            nn.Conv2d(2048, 1024, 1),
            nn.BatchNorm2d(1024),
            nn.ReLU()
        )
        self.classifier_p = nn.Linear(1024, num_classes)
        self._init_params()
    ...
    def forward(self, x):
        ...
        #global branch
        global_feature = self.global_avgpool(x) # ([32, 2048, 1, 1])
        global_triplet_feature = self.reduction_g(global_feature) # ([32, 512, 1, 1])
        global_triplet_feature = global_triplet_feature.view(global_triplet_feature.size(0), -1)
        global_softmax_class = self.classifier_g(global_triplet_feature)
        ...
        x = self.res_part2(x)
        x = self.batch_drop(x)
        part_feature = self.part_maxpool(x) # ([32, 2048, 1, 1])
        part_triplet_feature = self.reduction_p(part_feature) # ([32, 1024, 1, 1])
        part_triplet_feature = part_triplet_feature.view(part_triplet_feature.size(0), -1)
        part_softmax_class = self.classifier_p(part_triplet_feature)
        ...
        if self.loss == 'softmax':
            ...
            return softmax_features, triplet_features
        else:
