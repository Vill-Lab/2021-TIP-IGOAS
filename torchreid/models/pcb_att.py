...
class PartAttn(nn.Module):
    """Spatial Attention (Sec. 3.1.I.1)"""
    
    def __init__(self):
        super(PartAttn, self).__init__()
        self.conv = ConvBlock(1, 1, 1)

    def forward(self, x): #([32, 2048, 6, 1]) 
        # global cross-channel averaging
        input = x
        x = x.mean(1, keepdim=True)
        x = self.conv(x)
        part_att = torch.softmax(x,dim=1)        
        return input * part_att
...
class PCB(nn.Module):
    ...
    def forward(self, x):
        ...
        v_g = self.parts_avgpool(f) #([32, 2048, 6, 1])  
        v_g_mask = self.partattn(v_g) #([32, 2048, 6, 1])
        v_g_all = v_g + v_g_mask      
        
        if not self.training:
            v_g = F.normalize(v_g_all, p=2, dim=1)
            return v_g.view(v_g.size(0), -1) #[100, 12288=2048*6]

        v_g = self.dropout(v_g_all) #([32, 2048, 6, 1])        
        v_h = self.conv5(v_g) #([32, 256, 6, 1])
        
        y = []
        for i in range(self.parts):
            v_h_i = v_h[:, :, i, :]
            v_h_i = v_h_i.view(v_h_i.size(0), -1) #([32, 256])
