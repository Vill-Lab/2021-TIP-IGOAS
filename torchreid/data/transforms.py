...
def build_transforms(height, width, transforms='random_flip', norm_mean=[0.485, 0.456, 0.406],
    ...
    if 'color_jitter' in transforms:
        print('+ color jitter')
        transform_tr += [ColorJitter(brightness=0.25, contrast=0.15, saturation=0.25, hue=0)]  # HONet setting
    ...
    transform_te = Compose([
        Resize((height, width)),
        # ColorJitter(brightness=0.25, contrast=0.15, saturation=0.25, hue=0),  # test on O/P dataset
        ToTensor(),
        normalize,
    ])
